...
guard CommandLine.arguments.count == 3 || CommandLine.arguments.count == 4 else {
    ...
    print("[i] \(CommandLine.arguments[0]) /path/to/location.csv /path/to/photo/dir <optional>any(for overwrite)")
    print("eg. \(CommandLine.arguments[0]) /path/to/location.csv /path/to/photo/dir")
    print("eg. \(CommandLine.arguments[0]) /path/to/location.csv /path/to/photo/dir 1")
    exit(1)
}

let gpsFile = URL(fileURLWithPath: CommandLine.arguments[1])
let searchDir = URL(fileURLWithPath: CommandLine.arguments[2])
let overwrite = CommandLine.arguments.count == 4
...
for (idx, url) in candidates.enumerated() {
    print("[*] processing \(idx.paddedString(totalLength: paddingLength))/\(candidates.count) <\(url.lastPathComponent)>: ", separator: "", terminator: "")
    fflush(stdout)
    autoreleasepool {
...
extension CSV {
    ...
    public func enumerateAsArray(limitTo maxRow: Int? = nil, startAt: Int = 0, _ rowCallback: @escaping ([String]) -> Void) throws {
    ...
    public func enumerateAsArray(startAt: Int = 0, rowLimit: Int? = nil, _ rowCallback: @escaping ([String]) -> Void) throws {
        try Parser.enumerateAsArray(text: text, delimiter: delimiter, startAt: startAt, rowLimit: rowLimit, rowCallback: rowCallback)
    }

    public func enumerateAsDict(_ block: @escaping ([String: String]) -> Void) throws {
